...
pushd "%~dp0%WCROOT%" && (
  rem reinit git svn
  call :GIT_SVN_INIT "%%ORBITTOOLS_DEPLOY.SVN.REPOROOT%%" || ( popd & goto EXIT )

  call :CMD git config user.name "%%GIT2.USER%%" || ( popd & goto EXIT )
  call :CMD git config user.email "%%GIT2.EMAIL%%" || ( popd & goto EXIT )

  (
    git remote get-url origin > nul 2> nul && call :CMD git remote set-url origin "%%ORBITTOOLS_DEPLOY.GIT2.ORIGIN%%"
  ) || call :CMD git remote add origin "%%ORBITTOOLS_DEPLOY.GIT2.ORIGIN%%" || ( popd & goto EXIT )
