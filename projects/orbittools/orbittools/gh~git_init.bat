...
pushd "%~dp0%WCROOT%" && (
  ...
  (
    git remote get-url origin > nul 2> nul && call :CMD git remote set-url origin "%%ORBITTOOLS_ORBITTOOLS.GIT.ORIGIN%%"
  ) || call :CMD git remote add origin "%%ORBITTOOLS_ORBITTOOLS.GIT.ORIGIN%%" || ( popd & goto EXIT )
