...
pushd "%~dp0%WCROOT%" && (
  rem reinit git svn
  call :GIT_SVN_INIT "%%ORBITTOOLS_ORBITTOOLS.SVN.REPOROOT%%" --stdlayout || ( popd & goto EXIT )

  call :CMD git config user.name "%%GIT2.USER%%" || ( popd & goto EXIT )
  call :CMD git config user.email "%%GIT2.EMAIL%%" || ( popd & goto EXIT )

  (
    git remote get-url origin > nul 2> nul && call :CMD git remote set-url origin "%%ORBITTOOLS_ORBITTOOLS.GIT2.ORIGIN%%"
  ) || call :CMD git remote add origin "%%ORBITTOOLS_ORBITTOOLS.GIT2.ORIGIN%%" || ( popd & goto EXIT )
