...
pushd "%~dp0%WCROOT%" && (
  ...
  (
    git remote get-url origin > nul 2> nul && call :CMD git remote set-url origin "%%ORBITTOOLS_SGP4.GIT.ORIGIN%%"
  ) || call :CMD git remote add origin "%%ORBITTOOLS_SGP4.GIT.ORIGIN%%" || ( popd & goto EXIT )
